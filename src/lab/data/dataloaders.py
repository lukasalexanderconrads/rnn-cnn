...
class DataLoaderCifar100:
    def __init__(self, device: torch.device, valid_fraction: float = .1, batch_size: int = 1):
        ...
        train_dataset = CIFAR100(root='data/', transform=ToTensor(), download=True)
        data_len = len(train_dataset)
        train_len = int(data_len * (1 - valid_fraction))
        test_len = data_len - train_len
        train_set, test_set = random_split(train_dataset, [train_len, test_len])
        ...
        self.test = DataLoader(CIFAR100(root='data/', train=False, transform=ToTensor()), batch_size=batch_size)
